...
class NetworkPlanner:
    def __init__(self, num_nodes, num_links, required_compute_nodes=2, min_computing_power=0, min_bandwidth=0):
        ...
        for u, v in tree.edges():
            ...
            # 向图中添加边，并设置传播时延和带宽属性
            self.G.add_edge(u, v, propagation_delay=propagation_delay, bandwidth=bandwidth)
    ...
    def ant_colony_optimization(self, source, destination, num_computing_nodes,min_computing_power,min_bandwidth,packet_size=100, num_ants=20, max_iter=50,
        ...
        :param beta: 启发式因子
        :param rho: 信息素挥发因子
        :return: 最短时延路径，如果不存在满足条件的路径则返回 None
        """
...
if __name__ == "__main__":
    ...
    if path:
        min_delay = planner.calculate_total_delay(path, packet_size, num_computing_nodes)
        print("找到的路径：", path)
        print("中间节点：", select_node)
        print("最小时延是",min_delay)
        end_time = time.time();
