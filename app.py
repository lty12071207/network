...
def debugforroute():
    #topology_data=convert_json_to_echarts_topology('./api/testtopo.json','./api/node_config.json')
    createroutejson('./api/intent/i1.json')
    route = load_json_file('./api/route.json')
    print(route)
    topology_data = drawroute('./api/testtopo.json', './api/node_config.json', route)
    return render_template('test2.html', topology_data=topology_data)
